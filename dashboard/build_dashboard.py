...
Free GeckoTerminal + Solana RPC + Helius data, on a committed Dexscreener universe | snapshot 2026-06-21 | {S['n_pools']} pools screened</div></header>
<main>
<p class="lead">A wallet-level detector scored the liquid xStock pools on Solana. <strong>{S['n_flagged']}</strong> exhibit a wash-trading signature: automated wallets that buy and sell in matched size and alternate buy and sell at a near-fixed cadence; in three of the five the wallets also trace to shared funding. Every other pool, including the same xStocks in their other pools, scores at most {S['max_nonflagged_score']:.2f}. A flag on the on-chain pattern, not on any identified person; each claim is a named wallet and a transaction hash.</p>
...
<div class="grid">
  ...
  <div class="stat bad"><div class="label">Lifetime matched</div><div class="value">${L['total_matched_usd']/1e6:.1f}M</div><div class="sub">{L['total_swaps']:,} swaps over the 14 wallets' history in SPYX/TSLAX/QQQX; one wallet ${L['max_bot_matched_usd']/1e6:.1f}M</div></div>
</div>
...
<li><b>Not cross-pool arbitrage, though the swaps are routed.</b> No bot transaction buys in one pool and sells in another, while {R['mixed_side_tx_in_snapshot']} transactions in the snapshot do exactly that, by other wallets - and no bot is buy-heavy in one pool while sell-heavy in another, which is what arbitraging two pools minutes apart would leave behind. The bots' swaps are often split by an aggregator across pools inside one transaction ({R['bot_routed_usd_share']:.0%} of their volume), so a pool tape shows a leg, not a whole swap - the tape still records the wallet as its own trade's sender, and each is a System-Program-owned account, not a program or router.</li>
<li><b>The bots run on a robot's clock.</b> That wallet's 72 swaps alternate buy/sell with no exception; its running signed trade value sawtooths between zero and about $300, back to near zero after every pair; the token position itself nets flat. Three TSLAX wallets run the same 4-5 second cadence back-to-back.</li>
<li><b>A coordinated fleet, where the funding says so.</b> Seven TSLAX wallets form a creation chain - the five wash bots and the two that funded them. Every wallet after the first was created and seeded by the one before it; the six seeds near a constant 500, falling in steps of 4.3 to 4.6 (the trace records the amount, not the mint). Three SPYX wallets run the same 7-and-7 buy/sell pattern for near-identical dollars (~$12,220 each); counting only its own transactions, not the legs of routed swaps, one wallet round-trips SPYX in all three of its pools.</li>
<li><b>It is the pool, not the token.</b> QQQX carries a sustained wash fleet in one Raydium pool and none in another of the same mint; TSLAX is flagged on Orca and below the flag on Raydium (the discriminator is the sustained fleet, since share is window-sensitive). The bots predate the pools (earliest {T['earliest_bot']}).</li>
...
<li><b>The snapshot caught a sliver.</b> Across the fourteen wallets' history - classified against the seven xStock mints the collector knows, of which these wallets touch three (SPYX, TSLAX, QQQX) - the two-sided matched total is ${L['total_matched_usd']/1e6:.1f}M in {L['total_swaps']:,} swaps (one wallet alone ${L['max_bot_matched_usd']/1e6:.1f}M), each wallet's washing concentrated in a multi-day burst - more than ten times the in-window floor, and a floor itself since any xStock below the volume cutoff is not counted.</li>
</ol></section>
<section><h2>The screen: the liquid Solana xStock pools</h2><div class="sub">Wash score = USD share transacted by wallets that buy and sell the pool &ge;5x each within 10% of flat. Five flagged, calibrated above every organic control.</div><div class="figure"><img src="data:image/png;base64,{fig['screen.png']}" alt="wash score per scored Solana xStock pool: five flagged in orange above the flag line; the WIF and JUP organic controls are marked in green at zero"></div>
...
<div class="tnote">window = time span of the most-recent-300-swap snapshot; wash% is measured within it.</div>
...
<div class="tnote">Window confound: wash share is window-sensitive - sliced to the shortest flagged window (~{S['subwindow_robustness']['equal_window_h']*60:.0f} min), {S['subwindow_robustness']['n_nonflagged_subwindow_above_flag']} non-flagged pools show a lone round-tripper above the flag. The robust discriminator is the full-window fleet: no non-flagged pool carries more than {S['subwindow_robustness']['max_nonflagged_fullwindow_wash_bots']} balanced bot over its snapshot, while every flagged pool carries &ge; {S['subwindow_robustness']['min_flagged_n_bots']}; three of the five re-flag six hours later and, on a different three, the bots share funding, and the wallets cycle millions lifetime.</div></section>
...
<section><h2>The mechanism: a robot that ends every round trip flat</h2><div class="sub">Wallet C6FyA84D on QQQX: 72 swaps in perfect buy/sell alternation; the running signed trade value sawtooths to about $300 and back to near zero after each pair, while the token position itself nets flat (0.18% of the units it turned over, checked separately; fleet median 0.01%).</div><div class="figure"><img src="data:image/png;base64,{fig['cadence.png']}" alt="QQQX wallet alternates buy and sell across 72 swaps while its running signed trade value sawtooths between zero and about $300, returning to near zero after each pair"></div></section>
<section><h2>The coordination: a fleet built from one funding chain</h2><div class="sub">The TSLAX fleet as a creation chain: seven wallets, every one after the first created and seeded by the wallet before it, the six seeds falling in steps of 4.3 to 4.6. Grey funded the fleet; orange trades the pool.</div><div class="figure"><img src="data:image/png;base64,{fig['funding.png']}" alt="funding chain"></div></section>
...
<section><h2>The snapshot caught a sliver</h2><div class="sub">Each named bot's matched USD: the snapshot window against its history in SPYX/TSLAX/QQQX (log scale). The 14 wallets cycled ${L['total_matched_usd']/1e6:.1f}M in those three symbols, more than ten times the in-window floor.</div><div class="figure"><img src="data:image/png;base64,{fig['lifetime.png']}" alt="lifetime vs in-window matched per wallet"></div></section>
