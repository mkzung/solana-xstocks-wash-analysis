...
<li><b>The bots run on a robot's clock.</b> That wallet's 72 swaps alternate buy/sell with no exception; its net position sawtooths between zero and about $300, back to flat after every pair. Three TSLAX wallets run the same 4-5 second cadence back-to-back.</li>
...
<li><b>The snapshot caught a sliver.</b> Across the fourteen wallets' history in the three recognised xStocks (SPYX, TSLAX, QQQX) the directly-observed matched total is ${L['total_matched_usd']/1e6:.1f}M in {L['total_swaps']:,} swaps (one wallet alone ${L['max_bot_matched_usd']/1e6:.1f}M), each wallet's washing concentrated in a multi-day burst - more than ten times the in-window floor, and a floor itself since other xStocks are not counted.</li>
</ol></section>
...
<section><h2>The mechanism: a robot that ends every round trip flat</h2><div class="sub">Wallet C6FyA84D on QQQX: 72 swaps in perfect buy/sell alternation; the position is taken on each buy and given back on the next sell, ending the window flat.</div><div class="figure"><img src="data:image/png;base64,{fig['cadence.png']}" alt="cadence"></div></section>
